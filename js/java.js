...
const API = "https://shining-bell-86d3c62bcf.strapiapp.com/api";
    ...
    if (data.length > 0) {
      ...
      data.map((product, id) => {
        row.innerHTML += `<tr>
                ...
                <td class="status"><span class="${product.productStatus}">${
          product.productStatus ? "Bor" : "Yo'q"
        }</span></td>
                <td>
                ...
                <td>
                  ...
                  <div class="options">
                    ...
                    <div class="options_content">
                      <button class='options_content_edit' onclick="Edit('${
                        product.documentId
                      }')">
                        Edit 
                      <span>
                      ...
                      <button class='options_content_del' onclick="Delete('${
                        product.documentId
                      }')">
                        Delete 
                          <span>
      ...
      row.innerHTML =
        "<tr><td colspan='7'>Maxsulotlar hali kiritilmagan</td></tr>";
    }
...
function Edit(id) {
  ...
  fetch(`${API}/products/${id}`)
    ...
    .then((res) => {
      ...
      document.querySelector(".form_modal_edit #productName").value =
        productData.productName;
      document.querySelector(".form_modal_edit #productCode").value =
        productData.productId;
      document.querySelector(".form_modal_edit #description").value =
        productData.description;
      document.querySelector(".form_modal_edit #productStatus").value =
        productData.productStatus;
      document.querySelector(".form_modal_edit #purePrice").value =
        productData.purePrice;
      document.querySelector(".form_modal_edit #higherPrice").value =
        productData.higherPrice;
      document.querySelector(".form_modal_edit #Price").value =
        productData.price;
    })
    .catch((error) => {
      console.log(error);
...
document
  .querySelector(".header_modal_close_edit")
...
// Edit Modalni yopadi Yopish tugmasi bosilganda
document
  .querySelector(".header_modal_close_2_edit")
  .addEventListener("click", function () {
    document.querySelector(".modal_box_edit").classList.remove("active_modal");
    document
      .querySelector(".content_modal_box_edit")
...
function EditProduct(event) {
  ...
  fetch(`${API}/products/${ProductIdValue}`, {
    ...
    .then((data) => {
      console.log(data);
      document
        .querySelector(".modal_box_edit")
        .classList.remove("active_modal");
      document
        .querySelector(".content_modal_box_edit")
        .classList.remove("active_content_modal");
      GetProducts();
...
document.addEventListener("click", function (event) {
  ...
  document.querySelectorAll(".options_content").forEach((menu) => {
    if (
      ...
      !event.target.classList.contains("options_icon")
    ) {
...
function OpenOptions(id) {
  ...
  let menu =
    document.querySelectorAll("tr")[id + 1].children[6].children[1].children[1];
...
// Qo'shish modali ichidagi qo'shish tugmasi bosilganda Ma'lumotlarni strapiga qo'shadi va ekrani yangilab mahsulotlarni ekranga ko'rsatadi.
...
function Search(event) {
  ...
  try {
    fetch(
      `${API}/products?filters[productName][$containsi]=${value}&populate=*`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.data.length > 0) {
          ...
          row.innerHTML =
            "<td colspan='7' class='notFound'>Mahsulot Topilmadi</td>";
        }
      })
...
//filter funksiyasi bosilganda maxsulotlar alfabit bo'yicha tartiblanadi
// Filter
